...
export default {
  ...
  watch: {
    ...
    lng(newVal, oldVal) {
      console.log("this is new value", newVal, "this is old value", oldVal);
    }
  },
  created() {
    this.getSiteInfo();
  ...
  methods: {
    ...
    setUpClickListener() {
      this.map.addEventListener("tap", (evt) => {
        ...
        this.$emit("onClick", {
          lat: coord.lat.toFixed(7),
          long: coord.lng.toFixed(7)
        });
      });
    },
    ...
    addSites() {
      ...
      const homeIcon = require("../assets/icons/home-address@3x.png");
      homeElement.innerHTML = `<img src='${homeIcon}' style="width:10%">`;
      ...
      this.sites.forEach((site) => {
        ...
        if (site.latitude == this.lat && site.longitude == this.lng) {
          ...
          iconUrl = require("../assets/icons/MapPin@3x.png");
        }
        ...
        element.innerHTML = `<img src='${iconUrl}' style="width:8%">`;
    ...
    addMarkers() {
      ...
      this.markers.forEach((marker) => {
        const element = document.createElement("div");
        const iconUrl = require("../assets/icons/MapPin@3x.png");
        element.style.backgroundColor = "#ffffff";
        element.style.width = "32px";
        element.style.height = "32px";
        element.style.display = "block";
        ...
        element.style.boxShadow = "1px 1px 1px 1px rgba(38, 78, 238, 1)";
        element.innerHTML = `<img src='${iconUrl}' style="width:100%">`;
        const domIcon = new H.map.DomIcon(element, {
          onAttach(clonedElement, domIcon, domMarker) {},
          onDetach(clonedElement, domIcon, domMarker) {}
        });
